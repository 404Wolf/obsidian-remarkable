...
export default class MyPlugin extends Plugin {
  ...
  tryInsertingDrawing = async (orientation: Orientation): Promise<void> => {
    ...
    try {
      ...
      console.log("Calling resnap with...", {
        reSnapPath: this.settings.reSnapPath,
        rmSshKeyAddress: this.settings.rmSshKeyAddress,
        outputFilePath,
        postprocessor: this.settings.postprocessor,
        orientation,
      });
      await callReSnap(
        ...
        outputFilePath,
        this.settings.postprocessor,
        [
          ...(orientation === Orientation.PORTRAIT ? [] : ["-l"]),
          ...this.settings.reSnapExtraArgs,
        ],
      );
